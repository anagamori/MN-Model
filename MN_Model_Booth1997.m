...
plot(time,I_app)
xlabel('Time (s)')
ylabel('Applied current (microA/cm^2)')
subplot(3,1,2)
plot(time,V_s_vec)
xlabel('Time (s)')
...
plot(time,V_s_vec);
ax2 = subplot(2,1,2);
plot(time,-g_Na*m_inf_vec.^3.*h_vec.*(V_s_vec-V_Na));
hold on 
plot(time,-g_K_dr*n_vec.^4.*(V_s_vec-V_K))
plot(time,-g_Ca_N*m_N_s_vec.^2.*h_N_s_vec.*(V_s-V_Ca))
plot(time,-g_K_Ca*Ca_s_vec./(Ca_s_vec+K_d).*(V_s-V_K))
xlabel('Time (s)')
ylim([-300 300])
