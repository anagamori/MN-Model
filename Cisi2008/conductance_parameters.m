...
mnParameter.g_Kf = 4; %[4*ones(index_slow,1);0.5*ones(N_MU-index_slow,1)]; %*param_s.area_s ; %(mS/cm^2)
mnParameter.g_Ks = 16; %[16*ones(index_slow,1);4*ones(N_MU-index_slow,1)]; %*param_s.area_s ; %(mS/cm^2)
mnParameter.g_c = 0.1;
%%
...
mnParameter.beta_q = 0.025*1000; %[0.025*1000*ones(index_slow,1);0.05*ones(N_MU-index_slow,1)];
